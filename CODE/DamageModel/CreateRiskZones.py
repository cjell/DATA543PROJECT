...
risk_zones = dissolved_polygons.explode(ignore_index=True)
...
for riskiness, color in colors.items():
    subset = risk_zones[risk_zones["risk_level"] == riskiness]
    subset.plot(
        ax=ax,
        facecolor=color,
        edgecolor="grey",
...
ax.set_title("Wildfire Risk Zones")
ax.set_axis_off()
plt.tight_layout()
legend_elements = [
...
risk_zones.to_file("CreatedData/RiskZones/risk_zones.shp")
